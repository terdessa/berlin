...
function AudioPage() {
  // Render a stable SSR skeleton until
...
function AudioPageInner() {
  ...
  const talkDesiredRef = useRef(false);
  ...
  const setTalkEnabled = async (enabled: boolean) => {
    const generation = talkGenerationRef.current + 1;
    talkGenerationRef.current = generation;
    talkDesiredRef.current = enabled;
    ...
    if (!enabled) {
      ...
      if (generation === talkGenerationRef.current) setMicOn(false);
    } else {
      if (stream) {
        for (const t of stream.getAudioTracks()) t.enabled = true;
      }
      ...
      if (talkDesiredRef.current && generation === talkGenerationRef.current) {
        setMicOn(true);
        return;
      }
      if (lkTrack) await lkTrack.mute().catch(() => {});
      if (stream) {
        for (const t of stream.getAudioTracks()) t.enabled = false;
      }
    }
  ...
  const canTalk = status.state === "connected";
  ...
  const startTalking = () => {
    ...
    void setTalkEnabled(true);
  };

  const stopTalking = () => {
    if (!canTalk) return;
    void setTalkEnabled(false);
  };
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
        <div>
          <h1 className="mono text-sm uppercase tracking-[0.2em] text-foreground">
            Sentinel · walkie-talkie mic
          </h1>
          <p className="mt-1 text-xs text-muted-foreground">
            Direct-link page · publishes this guard microphone to the LiveKit voice room.
          </p>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-panel">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border bg-panel-elevated px-4 py-3">
            <button
              ...
              disabled={!canTalk}
              className={[
                "mono select-none rounded-md border px-4 py-2 text-xs transition disabled:cursor-not-allowed disabled:opacity-40",
                micOn
            ...
            >
              {micOn ? "transmitting · release" : "hold to talk"}
            </button>
