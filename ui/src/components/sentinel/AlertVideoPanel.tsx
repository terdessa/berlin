...
export function AlertVideoPanel({ alert }: Props) {
  ...
  if (!alert) {
    return (
      <div className="flex h-full flex-col items-center justify-center rounded-lg border border-border bg-panel/60 text-center">
  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden bg-background">
        ...
        {/* Top overlay */}
        <div className="absolute inset-x-0 top-0 flex items-center justify-between p-2.5 text-xs">
          <div className="flex items-center gap-2">
            <span className="mono rounded-sm bg-alert px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-alert-foreground">
              ● requires review
            </span>
            <span className="mono rounded-sm bg-background/70 px-2 py-0.5 text-foreground/90 backdrop-blur-sm">
              {alert.cameraId} · {alert.zone}
            </span>
          </div>
          <span className="mono rounded-sm bg-background/70 px-2 py-0.5 text-muted-foreground backdrop-blur-sm">
            {alert.timestamp}
          </span>
        </div>
      ...
      {/* Controls */}
      <div className="border-t border-border bg-panel-elevated px-3 py-2">
        <div className="flex items-center gap-2">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex gap-1.5">
            <button className="mono cursor-pointer rounded-md border border-border bg-background/60 px-2.5 py-1 text-[11px] text-foreground transition hover:border-primary/50 hover:bg-background">
              ↺ replay 10s
            </button>
            <button className="mono cursor-pointer rounded-md border border-border bg-background/60 px-2.5 py-1 text-[11px] text-foreground transition hover:border-primary/50 hover:bg-background">
              ▶ live
